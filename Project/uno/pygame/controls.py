...
def player_choice_color():
    ...
    selected = None
    redraw_screen_menu_color(None)  # O(4)
    while True:
        (update, selected, turn_done) = player_LR_selection_color(selected) # O(1)
        if update:
            redraw_screen_menu_color(selected)  # O(4) ==> O(1)
        if turn_done:
            if selected == 0:
...
def player_choice_target(players):
    ...
    """
    selected = None
    redraw_screen_menu_target(players, None) # O(n)
    while True:
        (update, selected, turn_done) = player_LR_selection_target(players,
                                                                   selected) # O(1)
        if update:
            redraw_screen_menu_target(players, selected) # O(n)
        if turn_done:
